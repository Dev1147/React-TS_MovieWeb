...
// CORS 설정
// app.use(cors({
//     origin: 'http://localhost:5173', // 허용할 프론트엔드 주소
...
//Request 타입 검사
 type registerReq = Pick<IUser,'name' | 'email' | 'password' | 'creatdate'>

//사용자 등록 API
app.post('/api/users/register', async (req: Request<{}, {}, registerReq>, res: Response)=>{
    ...
    try{
        ...
        const user = new User(req.body); //사용자 정보

        user.creatdate = localDate;
        
        await user.save() //비동기 작업 처리: user.save()가 완료될 때까지 기다림
...
//Request 타입 검사
type loginReq = Pick<IUser, 'email' | 'password'>
//로그인 API
app.post('/api/users/login', async (req:Request<{},{},loginReq>, res:Response) => {
    ...
    try{
        ...
        return res.status(200).json({loginSuccess:true, message:'로그인 성공!', userId: loginUser._id})
...
//auth API
app.get('/api/users/auth', auth, async (req:Request, res:Response)=> {
    const user = (req as any).user;    // `user`를 `req.user`로 접근
...
//로그아웃API
app.get('/api/users/logout', auth, async (req:Request, res:Response)=>{
